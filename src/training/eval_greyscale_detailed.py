"""
eval_greyscale_detailed.py

Per-model detailed evaluation for all greyscale models.

For each model produces:
  1. Confusion matrix heatmap
  2. Per-class metrics bar chart (Precision, Recall, F1)

Plus cross-model charts:
  3. Per-class accuracy overlay (all models on one chart)
  4. Summary metrics comparison (Macro-F1, MCC, Weighted-F1, Accuracy)

Outputs saved under data/models/greyscale/detailed/.

Usage:
    python3 src/training/eval_greyscale_detailed.py
"""
...
MODELS = [
    ...
    ("cnn", "CNN (v1)",    MODEL_DIR / "cnn" / "best_model.pth", "cnn"),
...
# Colour palette per model (consistent across all charts)
MODEL_COLOURS = [
    "#2196F3", "#4CAF50", "#FF9800", "#E91E63",  # baseline
...
def load_model(arch: str, ckpt: dict, device: torch.device) -> nn.Module:
    ...
    model = MalwareCNN(num_classes) if arch == "cnn" else MalwareCNNv4(num_classes)
...
def plot_confusion_matrix(y_true: np.ndarray, y_pred: np.ndarray,
                           class_names: list[str], display_label: str,
                           out_path: Path):
    ...
    fig.suptitle(f"Greyscale {display_label} — Confusion Matrix", fontsize=14, fontweight="bold")
...
def plot_per_class_metrics(y_true: np.ndarray, y_pred: np.ndarray,
    ...
    fig.suptitle(f"Greyscale {display_label} — Per-Class Metrics", fontsize=14, fontweight="bold")
    ...
    # Bottom: False Negative Rate (1 - Recall) per class
...
# ---------------------------------------------------------------------------
def plot_cross_model_f1(all_results: list[dict], class_names: list[str], out_path: Path):
    """Heatmap: rows=models, cols=classes, values=F1(%)."""
    ...
    ax.set_title("Per-Class F1 Score (%) by Model  —  Greyscale", fontsize=13, fontweight="bold")
...
# ---------------------------------------------------------------------------
# Plot 4: Summary metrics bar chart (Accuracy, Macro-F1, Weighted-F1, MCC)
# ---------------------------------------------------------------------------
def plot_summary_metrics(all_results: list[dict], out_path: Path):
    ...
    fig.suptitle("Greyscale Models — Summary Metrics", fontsize=14, fontweight="bold")
    ...
    bars = ax.bar(x, mcc, width=0.5,
                  color=[MODEL_COLOURS[i] for i in range(len(labels))], zorder=3)
...
def main():
    ...
    print("Loading greyscale test dataset ...")
    ...
    for short, display, ckpt_path, arch in MODELS:
        ...
        # Per-model plots
        prefix = OUT_DIR / f"greyscale_{short}"
    ...
    if all_results:
        ...
        plot_cross_model_f1(all_results, class_names,
                            OUT_DIR / "greyscale_cross_model_f1_heatmap.png")
        plot_summary_metrics(all_results,
                             OUT_DIR / "greyscale_summary_metrics.png")
