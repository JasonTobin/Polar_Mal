...
V3 corrections over V2 Phase 1:
  - CosineAnnealingLR (no restarts) instead of CosineAnnealingWarmRestarts
    — warm restarts catastrophically reset LR mid-convergence in V2
  - Label smoothing reduced 0.1 -> 0.05 (lower noise for confident spiral features)
  - AdamW + weight_decay=1e-4 retained from V2
  - RandomErasing augmentation retained from V2
  - AMP + RAM dataset cache retained from V1/V2

Saves to data/models/v3/ — V1 and V2 model files are untouched.
"""
...
# ---------------------------------------------------------------------------
# CNN Architecture  (identical to V1/V2)
# ---------------------------------------------------------------------------
...
def main():
    ...
    # Label smoothing kept at 0.05 (halved from V2's 0.1 — less noise, still regularizes)
    criterion = nn.CrossEntropyLoss(label_smoothing=0.05)
    # AdamW retained from V2
    optimizer = optim.AdamW(model.parameters(), lr=LEARNING_RATE, weight_decay=1e-4)
    # Single cosine decay: LR smoothly falls 1e-3 -> 1e-6 over T_max epochs, no restarts
    ...
    for epoch in range(start_epoch, NUM_EPOCHS + 1):
        ...
        # Write CSV after every epoch so graphs can be generated mid-run
