"""
train_v4_greyscale_sampler.py

Imbalance strategy: WeightedRandomSampler only.
  - Per-sample weight = 1 / class_count[class_of_sample], so every class
    is drawn at the same expected rate per batch.  With num_samples equal
    to the full training set size, epoch length is unchanged.
  - CrossEntropyLoss uses NO class weighting — loss weighting is handled
    entirely by the sampler.  This isolates the sampling effect for ablation.
...
Saves to data/models/greyscale/v4_sampler/
"""
...
MODEL_DIR      = ROOT / "data" / "models" / "greyscale" / "v4_sampler"
...
def main():
    ...
    print(f"Imbalance strategy: WeightedRandomSampler only\n")
    ...
    # --- Build WeightedRandomSampler (sqrt of inverse frequency) ---
    # Linear 1/count oversamples cryptominer ~560x per epoch (from 14 unique images),
    # causing severe overfitting.  Sqrt reduces this to ~37x — still strong signal.
    ...
    print("Per-class sampling weights (proportional to 1/sqrt(class_count)):")
    for i, cls in enumerate(train_ds.classes):
        print(f"  [{i:2d}] {cls:<22} count={class_counts[i]:6d}  "
              f"sample_weight={1.0/(class_counts[i]**0.5):.6f}")
    print()

    pin = device.type == "cuda"
    # shuffle=False because sampler is mutually exclusive with shuffle
