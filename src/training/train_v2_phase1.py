...
Phase 1 training improvements over v1 (train_cnn.py).
Same MalwareCNN architecture; better training setup:

  - AdamW (weight_decay=1e-4) instead of Adam
  - CosineAnnealingWarmRestarts instead of StepLR — restarts escape local optima
  - CrossEntropyLoss(label_smoothing=0.1) — prevents overconfidence plateau
  - Class-weighted loss — corrects for imbalanced category counts
  - RandomErasing augmentation — forces robustness to partial features
  - AMP (FP16) + RAM dataset cache retained from v1

Saves to data/models/v2/ to preserve v1 model history independently.
"""
...
# ---------------------------------------------------------------------------
# Paths  (v2 lives in its own subdirectory)
# ---------------------------------------------------------------------------
ROOT         = Path(__file__).resolve().parents[2]
DATA_DIR     = ROOT / "data" / "images"
MODEL_DIR    = ROOT / "data" / "models" / "v2"
...
# ---------------------------------------------------------------------------
# Data transforms  (RandomErasing added vs v1)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# CNN Architecture  (identical to v1 — architecture unchanged in Phase 1)
# ---------------------------------------------------------------------------
