"""
train_v4_greyscale_balanced.py

Imbalance strategy: WeightedRandomSampler + Weighted CrossEntropyLoss (combined).
  - WeightedRandomSampler: per-sample weight = 1 / class_count so every class
    is drawn at equal expected rate per batch.  num_samples = len(train_ds),
    keeping epoch length identical to the baseline.
  - Weighted CrossEntropyLoss: w_i = total / (num_classes * count_i),
    providing an additional loss-level signal on top of the sampler.
  - This combination is the strongest imbalance correction available without
    data augmentation or oversampling beyond replacement=True.
...
Other changes vs train_v4.py:
  ...
  - DataLoader: shuffle=True replaced by sampler=WeightedRandomSampler(...)
  - All other hyperparameters (SWA, AdamW, cosine LR, AMP, etc.) are
    identical to train_v4.py.

Saves to data/models/greyscale/v4_balanced/
"""
...
MODEL_DIR      = ROOT / "data" / "models" / "greyscale" / "v4_balanced"
...
def main():
    ...
    print(f"Imbalance strategy: WeightedRandomSampler + Weighted CrossEntropyLoss\n")
    ...
    # --- Weighted CrossEntropyLoss (sqrt of inverse frequency) ---
    class_weights = torch.tensor(
        [(total_samples / (num_classes * class_counts[i])) ** 0.5 for i in range(num_classes)],
        dtype=torch.float,
    )

    # --- WeightedRandomSampler (sqrt of inverse frequency) ---
    sample_weights = [1.0 / (class_counts[t] ** 0.5) for t in train_ds.targets]
    ...
    print("Class sqrt-weights / sqrt-sampling info:")
    for i, cls in enumerate(train_ds.classes):
        print(f"  [{i:2d}] {cls:<22} count={class_counts[i]:6d}  "
              f"sqrt_ce_weight={class_weights[i]:.4f}  "
              f"sqrt_sample_weight={1.0/(class_counts[i]**0.5):.6f}")
    print()
